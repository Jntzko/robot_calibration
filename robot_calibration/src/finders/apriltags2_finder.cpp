...
#include <robot_calibration/capture/apriltags2_finder.h>
...
PLUGINLIB_EXPORT_CLASS(robot_calibration::AprilTags2Finder, robot_calibration::FeatureFinder)

namespace robot_calibration
{

AprilTags2Finder::AprilTags2Finder() :
  waiting_(false)
{
}

bool AprilTags2Finder::init(const std::string& name, ros::NodeHandle& nh)
{
  if (!FeatureFinder::init(name, nh))
    return false;

  tag_detector_ = new apriltags2_ros::TagDetector(nh);
  // Setup Subscriber
...
void AprilTags2Finder::cameraCallback(const sensor_msgs::Image::ConstPtr& image)
{
  if (waiting_)
  {
    image_ = *image;
...
bool AprilTags2Finder::waitForImage()
{
...
bool AprilTags2Finder::find(robot_calibration_msgs::CalibrationData * msg)
{
